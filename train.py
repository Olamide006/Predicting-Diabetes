...
for name,model in models.items():
    ...
    print(f"CV Train Weighted F1: {cv_results['train_f1_weighted'].mean():.4f}")
    print(f"CV Test  Weighted F1: {cv_results['test_f1_weighted'].mean():.4f}")
    gap = cv_results['train_f1_weighted'].mean() - cv_results['test_f1_weighted'].mean()
    print(f"Overfitting Gap:       {gap:.4f} {'⚠️ Possible overfit' if gap > 0.05 else '✅ No significant overfit'}")
